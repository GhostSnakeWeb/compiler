...
def code_gen(x):
    if x == None:
        ...
        p1 = hole()  # освобождаем место для jump
        code_gen(x.right.left)  # если true
        if (x.right.right != None):
            emit_byte(JMP)  # иначе перепрыгиваем
            p2 = hole()
        ...
        p2 = hole()
        code_gen(x.right)
        emit_byte(JMP)  # jump обратно к вершине
        ...
        error("ошибка генерации - найдено %d, ожидалось" % (x.node_type))
...
def list_code():
    ...
    while pc < len(code):
        ...
        if op == FETCH:
            ...
            error("list_code: Неизвестный оператор %d", (op));
...
if len(sys.argv) > 1:
    try:
        ...
        error("Невозможно открыть %s" % sys.argv[1])
