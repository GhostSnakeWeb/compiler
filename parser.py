...
all_syms = {"End_of_input": tk_EOI, "Op_multiply": tk_Mul,
            ...
            "Keyword_print": tk_Print, "LeftParen": tk_Lparen,
            "RightParen": tk_Rparen, "LeftBrace": tk_Lbrace,
            "RightBrace": tk_Rbrace, "Semicolon": tk_Semi,
            "Comma": tk_Comma, "Identifier": tk_Ident,
            "Integer": tk_Integer, "String": tk_String }
...
def gettok():
    ...
    if tok == None:
        error("Неизвестный токен %s" % (tok_text))
...
def expect(msg, s):
    if tok == s:
        gettok()
        return
    error("%s: Ожидалось '%s', найдено '%s'" % (msg, Tokens[s][TK_NAME], Tokens[tok][TK_NAME]))
...
def expr(p):
    ...
    if tok == tk_Lparen:
        ...
        error("Ожидалось простое, найдено: %s" % (Tokens[tok][TK_NAME]))
...
def stmt():
    ...
    if tok == tk_If:
        ...
        error("Найдено: %s" % (Tokens[tok][TK_NAME]))
