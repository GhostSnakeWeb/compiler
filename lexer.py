...
# Две следующих строки токенов и их названий в одинаковом порядке
...
tk_Lparen, tk_Rparen, tk_Lbrace, tk_Rbrace, tk_Semi, tk_Comma, tk_Ident, \
...
key_words = {'if': tk_If, 'else': tk_Else, 'print': tk_Print, 'while': tk_While}
...
# Следующий символ из входных данных
def next_ch():
...
# 'x' - символьные константы
def char_lit(err_line, err_col):
    n = ord(next_ch())  # Пропустить открывающую ковычку
    if the_ch == '\'':
        error(err_line, err_col, "пустая символьная константа")
    elif the_ch == '\\':
        next_ch()
        if the_ch == 'n':
            n = 10
            ...
            error(err_line, err_col, "неизвестная последовательность пробелов \\%c" % (the_ch))
    if next_ch() != '\'':
        error(err_line, err_col, "многосимвольная константа")
    next_ch()
    return tk_Integer, err_line, err_col, n


# Деление или комментарии
def div_or_cmt(err_line, err_col):
    ...
    while True:
        if the_ch == '*':
            ...
            error(err_line, err_col, "EOF в комментариях")
        else:
            next_ch()
...
def string_lit(start, err_line, err_col):
    ...
    while next_ch() != start:
        if len(the_ch) == 0:
            error(err_line, err_col, "EOF при сканировании строкового литерала")
        if the_ch == '\n':
            error(err_line, err_col, "EOL при сканировании строкового литерала")
        text += the_ch
...
# Обрабатываем идентификаторы и целые числа
def ident_or_int(err_line, err_col):
    ...
    if len(text) == 0:
        error(err_line, err_col, "ident_or_int: нераспознанный символ: (%d) '%c'" % (ord(the_ch), the_ch))

    if text[0].isdigit():
        if not is_number:
            error(err_line, err_col, "не число: %s" % (text))
        n = int(text)
        return tk_Integer, err_line, err_col, n
...
def follow(expect, ifyes, ifno, err_line, err_col):
    ...
    if ifno == tk_EOI:
        error(err_line, err_col, "неопознанный символ: (%d) '%c'" % (ord(the_ch), the_ch))

    return ifno, err_line, err_col


# Возвращаем следующий тип токена
def gettok():
...
if len(sys.argv) > 1:
    try:
        ...
        error(0, 0, "Невозможно открыть %s" % sys.argv[1])
